...
После публикации финальной записи отчёта local `main` и `origin/main` повторно сверяются и должны совпадать. Legacy local `master`, полностью содержащийся в `main`, также удалён безопасно через `git branch -d master`; других постоянных рабочих веток не остаётся.
